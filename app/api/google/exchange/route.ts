...
export async function POST(req: Request) {
  try {
    ...
    // 生成激活 token，可以根据你的实际需求处理生成方式，通常你可以通过系统生成激活链接或直接传递一个激活 token
    const activationToken = "generated-activation-token"; 

    // 激活用户并获取 accessToken
    const accessToken = await activateCustomer(customerId, activationToken, "googleAutoGeneratedPassword123");

    // 保存 accessToken（例如存储在 cookie 或 localStorage 中）
    return NextResponse.json(
